...
string Terminal::leia() {
    return leia(1024);
}

bool Terminal::terminado() const {
    int status;
    auto r = waitpid(pid, &status, WNOHANG);
    return r != 0;
}

string Terminal::leia(int N, char delim) {
    if (terminado()) return "";
    ...
    FD_SET(master, &fds);
    long flag0, flag1;
    ioctl(master, F_GETFL, &flag0);
    flag1 = flag0 |= O_NONBLOCK;
    ioctl(master, F_SETFL, &flag1);
    while (res.size() < N) {
        sched_yield();
        int nfds = select(master + 1, &fds, NULL, NULL, &tout);
        if (nfds > 0) {
            int n = read(master, &data, 1);

            if (n > 0) {
                if (delim && data == delim) break;
                res += data;
//                cout << "leia: data=" << res << endl;
            } else break;
        } else {
//            cout << "leia: FIM !" << endl;
            break;
...
vector<string> arqlines(const string & path) {
    ifstream arq(path);
    vector<string> v;
