...
char** Terminal::gera_argv(vector<string> &cmd) {
    if (cmd.empty()) throw -2;
    auto args = new char*[cmd.size() + 1];
    int j;
    for (j=0; j < cmd.size(); j++) {
        args[j] = strdup(cmd[j].c_str());
    }
    args[j] = nullptr;
...
void Terminal::exec(char **args) {
    ...
    //for (char *ptr = args[0]; ptr != nullptr; ptr++) delete ptr;
    delete[] args;
}

Terminal::Terminal(vector<string> & args) {
    auto argv = gera_argv(args);
    exec(argv);
}
